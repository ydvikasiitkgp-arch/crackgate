...
- `@answer-checker` (`.opencode/agents/answer-checker.md`): checkpoint-aware answer verification — the agent writes `<mock-id>-answer-report.json` + `<mock-id>-ledger.json` to `$TMPDIR/opencode/` after every batch and resumes from `lastCompletedId` on re-runs. For papers > 150 questions, if a single run stalls, invoke it in parallel chunks of 40–50 questions (return-only prompts, no file writes) and merge the results into one report.
