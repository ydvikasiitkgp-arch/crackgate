...
- `@answer-checker` (`.opencode/agents/answer-checker.md`): checkpoint-aware answer verification — the agent writes `<mock-id>-answer-report.json` + `<mock-id>-ledger.json` to `$TMPDIR/opencode/` after every batch and resumes from `lastCompletedId` on re-runs. Every fix carries a `kind` (wrong-answer | no-valid-option | ambiguous | premise-invalid | underdetermined). For papers > 150 questions, if a single run stalls, invoke it in parallel chunks of 40–50 questions (return-only prompts, no file writes) and merge the results into one report; tell each chunk to load the existing `<mock-id>-ledger.json` first so settled facts carry across chunks, and have each chunk return `difficultySummary` counts for its range so the merged report can sum them. Instruct chunks to skip-and-record stalled questions (max 5 assumption attempts + 1 web try per question) and to always return partial findings rather than an empty reply. If a chunk still stalls on a spiral-prone item (letter-code/analogy/seating puzzles), pre-seed that question's settled verdict into `<mock-id>-ledger.json` (`source: "caller-seeded (settled in earlier run)"`) and re-invoke — chunks must reuse ledger facts without re-deriving them.
