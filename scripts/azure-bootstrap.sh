#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────
# CrackGate Azure bootstrap — run ONCE on a fresh Azure Linux VM
# (Ubuntu 22.04/24.04, amd64 — e.g. Standard_B2s) as the `azureuser`
# provisioning account:
#     ssh azureuser@<public-ip>
#     export SSH_PUBKEY="ssh-ed25519 AAAA... you@laptop"
#     curl -fsSL https://raw.githubusercontent.com/iamyadavvikas/crackgate/main/scripts/azure-bootstrap.sh | sudo -E bash
#
# Cloud-specific notes (Azure):
#   • Azure NSG is the real firewall — OPEN inbound 80/tcp + 443/tcp on the
#     VM's network security group (portal → VM → Networking → Add inbound port
#     rule) BEFORE this script, or Let's Encrypt will fail later. UFW here is
#     defense-in-depth only.
#   • Ubuntu images log in as `azureuser` (root is locked). This script uses
#     sudo; it must NOT be run as root directly.
#   • Point your A record (crackgate.in / www) at this VM's public IP after
...
# ── 6. UFW (belt + suspenders alongside the Azure NSG) ──────────
log "Configuring UFW"
...
✅ Azure bootstrap complete.
  ...
  Public ports : 22, 80, 443 (UFW) — also verify the Azure NSG allows 80/443!

Next steps:
  1. exit
  2. Point your A record (crackgate.in / www) at this VM's public IP.
  3. Fill in the app secrets (Postgres, Auth, Razorpay, ...):
       ssh ${DEPLOY_USER}@<public-ip>
       nano /opt/crackgate/.env.production
  4. Add GitHub repo secrets (AZURE_HOST, AZURE_SSH_KEY, GHCR_USERNAME,
     GHCR_TOKEN with read:packages, ...) and vars — see deploy-azure.yml.
  5. Trigger "Deploy to Azure" from GitHub Actions (workflow_dispatch).
