...
if [[ -n "${GHCR_USERNAME:-}" && -n "${GHCR_TOKEN:-}" ]]; then
  ...
  if grep -q '^GHCR_USERNAME=' .env.production; then
    sed -i "s|^GHCR_USERNAME=.*|GHCR_USERNAME=\"${GHCR_USERNAME}\"|" .env.production
  else
    echo "GHCR_USERNAME=\"${GHCR_USERNAME}\"" >> .env.production
  fi
  if grep -q '^GHCR_TOKEN=' .env.production; then
    sed -i "s|^GHCR_TOKEN=.*|GHCR_TOKEN=\"${GHCR_TOKEN}\"|" .env.production
  else
    echo "GHCR_TOKEN=\"${GHCR_TOKEN}\"" >> .env.production
  fi
fi
...
# ── 3. Sync CI-managed values (only when non-empty so a missing CI var never
#        wipes a host-set value) ──────────────────────────────────
sync_env() {
  local key="$1" value="$2"
  [[ -z "${value}" ]] && return 0
...
GOT="$(docker inspect --format '{{.Config.Image}}' "$(${COMPOSE} ps -q web)")"
case "${GOT}" in
...
echo "✗ health check failed — rolling back to ${PREVIOUS_IMAGE_TAG}"
