...
import { NCL_SURVEYOR_MOCKS, NCL_SURVEYOR_PRICING } from "@/data/diploma/ncl-surveyor-mocks";
...
export const metadata = {
  title: "NCL Surveyor (Mining) Mock Tests · CrackGate",
  description:
    "20 full-length mock tests for NCL Surveyor T&S Gr. B CBT exam — 100 MCQs each, no negative marking, based on CMR 2017 and mine surveying syllabus.",
  alternates: { canonical: "/diploma/ncl/surveyor" },
};

const SYLLABUS = [
  "Linear Measurement & Chain Surveying",
  "EDM — Principles & Measurement",
  "Angular Measurement (Prismatic Compass)",
  "Plane Table Surveying & Contouring",
  "Miners' Dial & Compass Instruments",
  "Theodolite — Measurement & Traversing",
  "Total Station Surveying",
  "Levelling — Types, Booking & Reduction",
  "Tachometry",
  "Controlled Surveys (Triangulation, GPS)",
  "Field Astronomy & Gyro Theodolite",
  "Map Projections & National Grid",
  "Photogrammetry & Remote Sensing",
  "Theory of Errors & Adjustments",
  "Borehole Surveying & Geological Map Reading",
  "Mine Plans — Preparation & Legislation",
...
export default async function NclSurveyorPage() {
  const session = await auth();
  const userId = (session?.user as { id?: string } | undefined)?.id;
  const isAdmin = (session?.user as { role?: string } | undefined)?.role === "admin";
  const unlocked = isAdmin || (await hasEntitlement(userId ?? "", "DIPLOMA", "ncl-surveyor"));
  ...
  const attempts = userId
    ...
    : [];
  const completedIds = new Set(attempts.filter(a => a.refId.startsWith("diploma-ncl-surveyor-mock-")).map(a => a.refId));
  const attemptMap = new Map(attempts.map(a => [a.refId, { id: a.id, takenAt: a.takenAt }]));

  const liveCount = NCL_SURVEYOR_MOCKS.length;
  const payHref = "/pay/upi?plan=pro&exam=DIPLOMA&subject=ncl-surveyor";
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-[#1a3a5c] text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-5 py-14 lg:py-16">
          ...
          <div className="flex items-center gap-3 mt-4">
            <span className="badge border border-indigo-300/30 bg-indigo-300/10 text-indigo-300">
              T&S Gr. B · NCL Recruitment · 5 Posts
            </span>
          </div>
          ...
          <p className="mt-4 max-w-2xl text-white/80">
            20 full-length mock tests matching the NCL Surveyor CBT pattern — 100 MCQs, 100 marks,
            120 minutes, no negative marking. Section A (Technical 70Q) + Section B (General 30Q).
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <a href="#mocks" className="cg-neon inline-flex items-center gap-2 rounded-lg border border-indigo-400/70 bg-indigo-400/10 px-5 py-3 text-sm font-semibold text-indigo-100 transition hover:bg-indigo-400/20">
      ...
      <section id="mocks" className="max-w-7xl mx-auto px-5 py-14">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-extrabold text-ink">Surveyor (Mining) — {liveCount} Full-length Mock Tests</h2>
            <p className="mt-2 max-w-2xl text-sm text-muted">
              Complete NCL Surveyor series: <b>Section A: 70 Technical MCQs</b> (surveying, theodolite, total station, levelling) +{" "}
        ...
        {unlocked ? (
          <div className="mt-6 flex items-center gap-3 rounded-xl border border-ok/30 bg-ok/10 px-4 py-3 text-sm">
            ...
            <span className="font-semibold text-ink">Surveyor series unlocked.</span>
            <span className="text-muted">All {liveCount} mocks are open — start any set below.</span>
          </div>
        ) : (
          <div className="mt-6 overflow-hidden rounded-2xl border border-indigo-400/30 bg-gradient-to-r from-indigo-950 to-slate-900 text-white">
            <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
              <div className="flex items-start gap-3">
                <span aria-hidden className="mt-0.5 text-2xl">🔒</span>
                <div>
                  <h3 className="text-lg font-extrabold">Unlock all {liveCount} Surveyor mocks</h3>
                  <p className="mt-1 max-w-xl text-sm text-white/70">
                    Full official NCL Surveyor CBT pattern — {liveCount} complete 100-question papers
                    covering Technical Surveying + General sections. One payment, valid through the recruitment cycle.
                  </p>
                </div>
              </div>
              <div className="flex shrink-0 flex-col items-stretch gap-1 sm:items-end">
                <div className="text-right">
                  <span className="text-3xl font-extrabold">₹{NCL_SURVEYOR_PRICING.pro}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Link
                    href={payHref}
                    className="cg-neon inline-flex items-center justify-center gap-2 rounded-lg border border-indigo-400/70 bg-indigo-400/10 px-6 py-3 text-sm font-semibold text-indigo-100 transition hover:bg-indigo-400/20"
                  >
                    Unlock now <span aria-hidden>→</span>
                  </Link>
                  <AddToCartBtn exam="DIPLOMA" subject="ncl-surveyor" variant="light" size="md" />
                </div>
      ...
      <section className="border-t border-line bg-paper/40">
        <div className="max-w-3xl mx-auto px-5 py-12 text-center">
          <h3 className="text-lg font-bold text-ink">Get NCL Surveyor exam updates</h3>
          <p className="mt-1 text-sm text-muted">
            New mock releases, NCL notification alerts, and prep tips — once a week.
          </p>
          <div className="mt-4 flex justify-center">
            <NewsletterForm source="diploma-ncl-surveyor" />
          </div>
        </div>
      </section>
    </>
